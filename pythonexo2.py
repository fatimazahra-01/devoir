...
with open("data.txt", "rt") as f:
    data = f.read()
    print(f"Nombre total de mots : {compter_mots(data)}")
    print("Fréquence des mots :", frequences_mots(data))
    plus_frequents, nb_occ, moyenne = stats_mots(data)
    print(f"Mots les plus utilisés : {plus_frequents} ({nb_occ} fois)")
    print(f"Longueur moyenne des mots : {moyenne:.2f}")
    print("Palindromes :", trouver_palindromes(data))
    print("Nombre de phrases :", compter_phrases(data))
    print("Longueurs des phrases :", longueur_phrases(data))
    print("Ponctuation utilisée :", ponctuation_trouvee(data))
    print("Mots uniques :", mots_distincts(data))
    print("Top 10 des mots :", top_dix(data))
    print("Phrase la plus longue :", phrase_la_plus_longue(data))
    print(f"Diversité du vocabulaire : {diversite_vocabulaire(data):.2f}%")
    print("Patterns répétitifs :", patterns_repetitifs(data))
